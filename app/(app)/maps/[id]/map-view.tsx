  ...
  const { nodes, edges } = useMemo(() => {
    ...
    const flowEdges: Edge[] = graph.edges
      ...
      .map((e) => ({
        ...
        // the ones a link-only crawler never finds.
        animated: false,
        style:
          e.kind === 'button'
        ...
        labelStyle: {
          fill: e.kind === 'button' ? '#c4b5fd' : e.kind === 'redirect' ? '#fcd34d' : '#cbd5e1',
          fontSize: 10,
        },
  ...
  return (
    <div className="mt-4">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-semibold">{map.label || map.hostname}</h1>
          ...
          <p className="mt-1 text-sm text-slate-500">{map.targetUrl}</p>
        </div>
      ...
      <div className="mt-4 flex gap-4">
        <div className="h-[calc(100vh-320px)] min-h-[420px] flex-1 overflow-hidden rounded-xl border border-slate-800 bg-slate-950">
          {nodes.length === 0 ? (
            <div className="flex h-full items-center justify-center text-sm text-slate-500">
              {live ? 'Waiting for the first pages…' : 'No pages were mapped.'}
            </div>
          ) : (
            <ReactFlow
              nodes={nodes}
